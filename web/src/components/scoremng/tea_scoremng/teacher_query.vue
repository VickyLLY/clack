<template>
  <div>
    <div>
      ...
      <div class="year_semester">
        <button class="btn btn-default" type="submit" @click="tea_find_score">查询成绩</button>
      </div>
      <div class="year_semester">
        <button class="btn btn-link" type="submit" @click="tea_download_score">下载成绩</button>
      </div>
    </div>
      ...
      methods:{
        ...
        tea_download_score:function () {
          if(this.year=="" || this.semester==""){
            ...
            /*接口请求*/
            this.$http.post(this.Global_Api + '/scoremng/teacher_download_scores/',data).then((res) => {
              if (res.body.error_code !== 0) {
