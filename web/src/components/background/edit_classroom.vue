        ...
        <el-row>
          <el-col :span="24">
            <el-table :data="data_list" tooltip-effect="dark" style="width:100%" :default-sort="{prop:'create_time',order:'descending'}" @selection-change="get_multiple_selection">
              ...
              <el-table-column prop="classroom_id" label="教室ID" sortable>
              </el-table-column>
              <el-table-column prop="classroom_name" label="教室名称" sortable>
              </el-table-column>
              <el-table-column prop="classroom_capacity" label="教室容量" sortable>
              </el-table-column>
    ...
    methods: {
      ...
      add_submit: function() {
        this.$refs['add_form'].validate((valid) => {
          if(valid) {
            this.$http.post(this.Global_Api + '/background/add_classroom', {
      ...
      // 获取主界面数据
      get_data: function() {
        this.$http.post(this.Global_Api + '/background/classroom_list', {}).then(response => {
          let start = (this.current_page - 1)*this.page_size;
          let end  = start + this.page_size;
          if(end > response.data.classroom_list.length) {
            end = response.data.classroom_list.length;
          }
          this.data_list = response.data.classroom_list.slice(start, end);
          this.total = response.data.classroom_list.length;
        }).catch(error => {
          console.log(error);
        })
      },
      ...
      set_data: function(row) {
        ...
        this.edit_form.classroom_id = row.classroom_id;
        ...
        this.$refs['edit_form'].validate((valid) => {
          if(valid) {
            this.$http.post(this.Global_Api + '/background/edit_classroom', {
              user_name: this.user_name,
              user_token: this.user_token,
              classroom_id: this.edit_form.classroom_id,
      ...
      delete_single_data: function(row) {
        this.$confirm('删除后不可恢复，是否继续？', '警告', {
          ...
          this.$http.post(this.Global_Api + '/background/del_classroom', {
            user_name: this.user_name,
            user_token: this.user_token,
        ...
        this.$confirm('删除后不可恢复，是否继续？', '警告', {
          ...
          for(let i = 0; i < this.multiple_selection.length; ++i) {
            this.$http.post(this.Global_Api + '/background/del_classroom', {
              user_name: this.user_name,
              user_token: this.user_token,
              classroom_id: this.multiple_selection[i].classroom_id
            }).then(response => {
              if(response.body.error_code !== 0) {
                is_error = true;
              }
