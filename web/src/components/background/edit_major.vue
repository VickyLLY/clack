  ...
  <div>
        ...
        <el-row>
          <el-col :span="24">
            <el-table :data="data_list" tooltip-effect="dark" style="width:100%" :default-sort="{prop:'create_time',order:'descending'}" @selection-change="get_multiple_selection()">
              ...
              <el-table-column prop="major_id" label="专业ID" sortable>
              </el-table-column>
              <el-table-column prop="major_name" label="专业名称" sortable>
              </el-table-column>
              <el-table-column prop="major_department_id" label="所属学院ID" sortable>
              </el-table-column>
    ...
    <el-dialog title="添加" :visible.sync="add_dialog" @close="reset_form('add_form')">
      <el-form :model="add_form" :rules="data_rules" ref="add_form" label-width="100px">
        <el-form-item label="专业名称" prop="major_name">
          <el-input type="text" v-model="add_form.major_name" auto-complete="off">
          </el-input>
        </el-form-item>
        <el-form-item label="所属学院ID" prop="major_department_id">
          <el-input type="text" v-model="add_form.major_department_id" auto-complete="off">
          </el-input>
        </el-form-item>
        <el-form-item>
    ...
    <el-dialog title="编辑" :visible.sync="edit_dialog" @close="reset_form('edit_form')">
      <el-form :model="edit_form" :rules="data_rules" ref="edit_form" label-width="100px">
        <el-form-item label="专业名称" prop="major_name">
          <el-input type="text" v-model="edit_form.major_name"></el-input>
        </el-form-item>
        <el-form-item label="所属学院ID" prop="major_department_id">
          <el-input type="text" v-model="edit_form.major_department_id"></el-input>
        </el-form-item>
        <el-form-item>
  ...
  export default {
    ...
    data() {
      return {
        ...
        data_rules: {
          major_name: [
            { required: true, message: '请输入专业名称', tigger: 'blur' },
    ...
    methods: {
      ...
      add_submit: function() {
        this.$refs['add_form'].validate((valid) => {
          if(valid) {
            this.$http.post(this.Global_Api + '/background/add_major', {
              user_name: this.user_name,
              user_token: this.user_token,
              major: {
                major_name: this.add_form.major_name,
                major_department_id: parseInt(this.add_form.major_department_id)
              }
            }).then(response => {
      ...
      // 获取主界面数据
      get_data: function() {
        this.$http.post(this.Global_Api + '/background/major_list', {}).then(response => {
          let start = (this.current_page - 1)*this.page_size;
          let end  = start + this.page_size;
          if(end > response.data.major_list.length) {
            end = response.data.major_list.length;
          }
          this.data_list = response.data.major_list.slice(start, end);
          this.total = response.data.major_list.length;
        }).catch(error => {
          console.log(error);
        })
      },
      ...
      set_data: function(row) {
        ...
        this.edit_form.major_id = row.major_id;
        this.edit_form.major_department_id = row.major_department_id;
        this.edit_form.major_name = row.major_name;
      },
      // 提交编辑窗口数据
      edit_submit: function() {
        this.$refs['edit_form'].validate((valid) => {
          if(valid) {
            this.$http.post(this.Global_Api + '/background/edit_major', {
              user_name: this.user_name,
              user_token: this.user_token,
              major_id: parseInt(this.edit_form.major_id),
              major_name: this.edit_form.major_name,
              major_department_id: parseInt(this.edit_form.major_department_id)
            }).then(response => {
              if(response.body.error_code === 0) {
        ...
        this.$confirm('删除后不可恢复，是否继续？', '警告', {
          ...
          for(let i = 0; i < this.multiple_selection.length; ++i) {
            this.$http.post(this.Global_Api + '/background/del_major', {
              user_name: this.user_name,
              user_token: this.user_token,
              major_id: this.multiple_selection[i].major_id
            }).then(response => {
              if(response.body.error_code !== 0) {
                is_error = true;
              }
